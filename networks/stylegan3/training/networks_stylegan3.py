...
class SynthesisNetwork(torch.nn.Module):
    ...
    def forward(self, ws, x, start, stop, **layer_kwargs):
        misc.assert_shape(ws, [None, self.num_ws, self.w_dim])
        ws = ws.to(torch.float32).unbind(dim=1)

        # if stop is None: stop = len(self.layer_names)
        if stop is None: stop = len(self.layer_names)
        if x is None: x = self.input(ws[0])

        for name, w in zip(self.layer_names[start:stop], ws[1:][start:stop]):
...
class Generator(torch.nn.Module):
    ...
    def forward(self, z, c, x=None, start=0, stop=None, truncation_psi=1, truncation_cutoff=None, update_emas=False, **synthesis_kwargs):
        ws = self.mapping(z, c, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, update_emas=update_emas)
        img = self.synthesis(ws, x=x, start=start, stop=stop, update_emas=update_emas, **synthesis_kwargs)
        return img
